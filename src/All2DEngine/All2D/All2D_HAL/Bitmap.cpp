...
{
/*	BITMAP	tmpBitmap;
	HBITMAP bmapHandle=(HBITMAP)LoadImage(NULL, fileName, IMAGE_BITMAP,0,0, LR_CREATEDIBSECTION | LR_LOADFROMFILE);

	if (bmapHandle==NULL)
		return false;

	GetObject( bmapHandle,  // handle to graphics object
	  sizeof(BITMAP),     // size of buffer for object information
	  &tmpBitmap// buffer for object information
	);

	Height=tmpBitmap.bmHeight;
	Width=tmpBitmap.bmWidth;
	Depth=32;
	Pitch=0;
	...
	BMap =new PIXEL[Width * Height];	// Speicher holen

	if (All2D_System::getRenderMode()==OPENGL)
	{
		for (int y=0;y<Height; y++)
			for (int x=0;x<Width; x++) {
				BMap[y*Width+x]=((( (((BYTE*)tmpBitmap.bmBits)[(Height-y-1)*tmpBitmap.bmWidthBytes+x*3 +2])<<BSHIFT) & BMASK )|
								 (( (((BYTE*)tmpBitmap.bmBits)[(Height-y-1)*tmpBitmap.bmWidthBytes+x*3 +1])<<GSHIFT ) & GMASK) |
								 (  (((BYTE*)tmpBitmap.bmBits)[(Height-y-1)*tmpBitmap.bmWidthBytes+x*3 +0]&0xff)>>RSHIFT) & RMASK);
			}
	}else{
		for (int y=0;y<Height; y++)
			for (int x=0;x<Width; x++) {
				BMap[y*Width+x]=((( (((BYTE*)tmpBitmap.bmBits)[(Height-y-1)*tmpBitmap.bmWidthBytes+x*3 +2])<<RSHIFT) & RMASK )|
								 (( (((BYTE*)tmpBitmap.bmBits)[(Height-y-1)*tmpBitmap.bmWidthBytes+x*3 +1])<<GSHIFT ) & GMASK) |
								 (  (((BYTE*)tmpBitmap.bmBits)[(Height-y-1)*tmpBitmap.bmWidthBytes+x*3 +0]&0xff)>>BSHIFT) & BMASK);
			}
	}
	CalcPixPerRow();
	DeleteObject(bmapHandle);
*/	return true;
}

bool CBitMap::Open(FILE *infile, int imageType, int size, int offset)
{
	...
	bool retVal=false;
	if (imageType==IMAGETYPE_PCX)
    retVal=LoadPCX(infile);
  else if (imageType==IMAGETYPE_JPG)
    retVal=LoadJPG(infile);
  else if (imageType==IMAGETYPE_PNG)
    retVal=LoadPNG(infile);
...
bool CBitMap::LoadJPG(FILE* infile)
{
/*	unsigned char a,r,g,b;
	...
	PIXEL * pDummy = new PIXEL[width*height*4];
	PIXEL * pTest=pDummy;
...
//    put_scanline_someplace(buffer[0], row_stride);
    for (int x=0;x<width;x++) {
      ...
      if (cinfo.output_components>2) {
			...
			if (All2D_System::getRenderMode()==OPENGL)
				*(pDummy++) =((b<<RSHIFT)& RMASK) | ((g<<GSHIFT)& GMASK) | ((r>>BSHIFT)& BMASK) ;
			else
				*(pDummy++) =((r<<RSHIFT)& RMASK) | ((g<<GSHIFT)& GMASK) | ((b>>BSHIFT)& BMASK) ;
  ...
  // Falls schon eine Bitmap reserviert war, löschen...
  if (BMap) {
		delete BMap;
		BMap=NULL;
	}

  BMap = (PIXEL*)pTest; // Pointer sind equalent
  Height = height;
  Width = width;
  Depth = DEPTH;
  CalcPixPerRow();
	...
	return true;
*/}
bool CBitMap::LoadPCX(FILE* infile)   //Load PCX-File and Converts to 32Bit Image
{
	bool retVal=true;
  ...
  Format=GetC(infile);     // Header Information lesen.....
  Version=GetC(infile);
  Kompression=GetC(infile);
  BitsPerPixel=GetC(infile);
  i=GetC(infile);      //0
  i=GetC(infile);    //0
  i=GetC(infile);      //0
  i=GetC(infile);    //0
  Width=GetC(infile);      //Breite
  Width+=GetC(infile)*256;
  Height=GetC(infile);   //Hoehe
  Height+=GetC(infile)*256;
  for (i=13; (i<66) && (feof(infile)==0);i++)
    readByte=GetC(infile);           //Palette Überspringen...
  Planes=GetC(infile);
  BytesPerRow=GetC(infile);
  BytesPerRow+=GetC(infile)*256;
  Pal=GetC(infile);
  Pal+=GetC(infile)*256;
  for (i=70; (i<128)&& (feof(infile)==0);i++)
    readByte=GetC(infile);           //Rest des Header Überspringen...
  RGB=Planes-1;
  Width++;
  Height++;
  PicMem = new char[(Width)*(Height)*4+256];    // Speicher berechnen... wieso..+256?
  Depth=32;
  CalcPixPerRow();
  BMap=(PIXEL*)PicMem;
  Clear(0);     // Die BitMap löschen, damit kein Müll verbleibt..
  MemPtr=PicMem;

  while ((feof(infile)==0)&&(LineCount<Height))
  {
    readByte=GetC(infile);
    ...
    if (PixelCount>=Width)              // Raster Line fertig?
    {
      PixelCount=0;//-=(Width);
      RGB--;                    // Neue Comonente (R,G oder B )
      if (RGB<0)
      {
        LineCount++;              // Neue RasterLine
        RGB=Planes-1;
      }
  ...
  // Converting this 32BMap to 16...

  BMap16=new PIXEL[Width * Height ];
  PIXEL * tmpPtr=BMap16;
  MemPtr=PicMem;
  for (i=0; i<(Width*Height);i++) //Übersetzen der Palette in RGBA
  {
    int b = *MemPtr++;
    int g = *MemPtr++;
    int r = *MemPtr++;
    int a = *MemPtr++;
    *tmpPtr++=((r<<RSHIFT)& RMASK) | ((g<<GSHIFT)& GMASK) | ((b>>BSHIFT)& BMASK) ;
  }
  delete PicMem;
...
bool CBitMap::LoadPNG(FILE* infile)
{
    /*
    png_byte sig[8];
...
//	image = array2<vec3ub>(w, h);
	...
	for(int i=0; i < w; i++)
		for(unsigned int j=0; j < h; j++)
		{
			if (All2D_System::getRenderMode()==OPENGL)
			{
				b=*(img + ((j)*row_bytes + i * bytesPerPixel));
				g=*(img + ((j)*row_bytes + i * bytesPerPixel+1));
				r=*(img + ((j)*row_bytes + i * bytesPerPixel+2));
				a=*(img + ((j)*row_bytes + i * bytesPerPixel+3));
			}else{
				r=*(img + ((j)*row_bytes + i * bytesPerPixel));
				g=*(img + ((j)*row_bytes + i * bytesPerPixel+1));
				b=*(img + ((j)*row_bytes + i * bytesPerPixel+2));
				a=*(img + ((j)*row_bytes + i * bytesPerPixel+3));
			}
			int tmp = 0 ;

			if (bytesPerPixel==4)
				tmp=((a<<ASHIFT) & AMASK) | ((r<<RSHIFT)& RMASK) | ((g<<GSHIFT)& GMASK) | ((b>>BSHIFT)& BMASK) ;
			if (bytesPerPixel==3)
				tmp=((r<<RSHIFT)& RMASK) | ((g<<GSHIFT)& GMASK) | ((b>>BSHIFT)& BMASK) ;

			if (tmp==0 && (r!=0 || g!=0 || b!=0))
				tmp=0x0001;
			if ((tmp &PIXELMASK)==TRANSPARENCE) tmp=STDTRANSCOLOR;	// AlphaPINK ausmaskieren...
